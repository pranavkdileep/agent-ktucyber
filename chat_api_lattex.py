...
def build_answer_prompt(query: str, bookid: str, markdown_content: str, imageobjects: list[dict]) -> str:
    ...
    return f"""
        ...
        Instructions:
        1. Organize the note with clear LaTeX structure using sectioning commands, concise bullet points, and tables where appropriate.
        2. Explain concepts clearly and concisely, using examples only when necessary.
        3. Include relevant images from the "Available Images" list by inserting the tag <image id="ImageID"> at the most appropriate place in the LaTeX content.
        ...
        6. Use valid LaTeX syntax (for example: \\section, \\subsection, itemize, tabular).
        7. The output must be pure LaTeX code (except for the <image> tags). Do not output markdown.
        8. Do not wrap the output in triple backticks or any code fences.
        9. The output should look like an actual study note: concise, to the point, and without unnecessary verbosity.
        """
...
if __name__ == "__main__":
    ...
    uvicorn.run("chat_api_lattex:app", host="0.0.0.0", port=int(os.getenv("PORT", "8000")), reload=False)
